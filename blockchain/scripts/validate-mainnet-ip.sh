...
# Скрипт проверки валидации IP для Mainnet
# Можно запустить на сервере 65.108.15.19 или через SSH

set -e

NETWORK="mainnet"
EXPECTED_IP="65.108.15.19"
SCAN_API="https://scan.sv-1.global.canton.network.sync.global"

echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   Проверка валидации IP-адреса $EXPECTED_IP (Mainnet)        ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
...
if [ "$CURRENT_IP" = "$EXPECTED_IP" ]; then
  echo "✅ Правильный IP для Mainnet"
else
...
if [ "${fail_count:-0}" -eq 0 ] && [ "${success_count:-0}" -gt 0 ]; then
    echo "✅ IP-адрес валидирован для Mainnet!"
    ...
    echo "   4. Получен MainNet allocation"
fi
echo ""
