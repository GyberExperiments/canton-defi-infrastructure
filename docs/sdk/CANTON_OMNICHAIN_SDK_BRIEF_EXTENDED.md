...
| Параметр | Значение |
|----------|----------|
| Хост | `<YOUR_CANTON_NODE_HOST>` |
| HTTP Ledger API | `http://<YOUR_CANTON_NODE_HOST>:30757` |
| gRPC Ledger API | `<YOUR_CANTON_NODE_HOST>:30501` |
...
pub async fn connect_ledger() -> Result<LedgerClient, LedgerError> {
    let endpoint = "http://<YOUR_CANTON_NODE_HOST>:30501";
...
```bash
# Health check
curl http://<YOUR_CANTON_NODE_HOST>:30757/health

# Ledger info
curl http://<YOUR_CANTON_NODE_HOST>:30757/api/v1/ledger/identity

# Parties
curl http://<YOUR_CANTON_NODE_HOST>:30757/api/v1/parties
```
...
# config/devnet.yaml
ledger_api:
  grpc_host: "<YOUR_CANTON_NODE_HOST>"
  grpc_port: 30501
  http_url: "http://<YOUR_CANTON_NODE_HOST>:30757"
  tls: false
  auth:
    enabled: false
    jwks_url: "http://<YOUR_CANTON_NODE_HOST>:8000/auth/v1/jwks"
```
...
```bash
# Ledger API
export CANTON_LEDGER_GRPC=<YOUR_CANTON_NODE_HOST>:30501
export CANTON_LEDGER_HTTP=http://<YOUR_CANTON_NODE_HOST>:30757
export CANTON_PARTICIPANT_ID=participant1
export CANTON_PARTY_ID=wealth_management_party

# Auth (Supabase)
export CANTON_AUTH_TOKEN=
export CANTON_AUTH_JWKS_URL=http://<YOUR_CANTON_NODE_HOST>:8000/auth/v1/jwks
...
pub fn init_tracer() -> Tracer {
    let tracer = opentelemetry_otlp::new_pipeline()
        .tracing()
        .with_endpoint("http://<YOUR_CANTON_NODE_HOST>:4317")
...
```bash
# HTTP API
curl -s http://<YOUR_CANTON_NODE_HOST>:30757/health

# gRPC services
grpcurl -plaintext <YOUR_CANTON_NODE_HOST>:30501 list
...
mod tests {
    ...
    async fn test_submit_command() {
        let client = LedgerClient::connect(
            "http://<YOUR_CANTON_NODE_HOST>:30501",
            "participant1"
        ).await.unwrap();
