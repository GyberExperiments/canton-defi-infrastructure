...
class EnhancedIntercomService {
  ...
  async updateOrderStatus(orderId: string, newStatus: string, additionalInfo?: string): Promise<boolean> {
    try {
      ...
      // Also update user's active order status
      const user = await this.findUserByEmail(conversations[0]?.contacts?.contacts?.[0]?.email);
      if (user) {
        await this.client.put(`/users/${user.id}`, {
          custom_attributes: {
            ...user.custom_attributes,
            active_order_status: newStatus,
            last_activity: new Date().toISOString()
          }
        });
      }
   ...
   */
  async getConversationAnalytics(timeframe: 'day' | 'week' | 'month' = 'week') {
    try {
