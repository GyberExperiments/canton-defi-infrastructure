...
2. 🏛️ **US Treasury Bills** - токенизированные казначейские облигации США
      ...
      {mode === 'standard' && (
        <div className="p-3 bg-amber-500/10 border border-amber-400/30 rounded-xl mb-4">
          <p className="text-amber-300 text-sm">
            ⚠️ Standard mode requires admin approval. Processing time: 1-24 hours.
            ...
            {isExpiringSoon && offer.status === 'OPEN' && (
              <div className="mt-4 p-3 bg-amber-500/10 border border-amber-400/30 rounded-lg flex items-center gap-2">
                ...
                <span className="text-amber-300 text-sm">
                  ⚠️ Offer expires in less than 1 hour
                </span>
              </div>
            )}
          </motion.div>
...
sequenceDiagram
    ...
    Modal-->>User: ⏱️ Awaiting approval (1-24h)
...
describe('TreasuryBillsPanel - Legacy Mode', () => {
  it('should switch to standard mode on toggle', () => {
    render(<TreasuryBillsPanel />);
    fireEvent.click(screen.getByText('⏱️ Standard (Approval)'));
    
    expect(screen.getByText('⏱️ Standard (Approval)')).toHaveClass('active');
  });
...
- ⏱️ Set `LEGACY_MODE_AVAILABLE=false` (hide toggle)
- ⏱️ Migrate remaining users
- ⏱️ Archive old purchase_requests
